...
peak_list_sub = read.table(paste0(work_path, "/tmp_", mamm, "/window_list_merged.txt"))
...
peak_list_sub_filter = read.table(paste0(work_path, "/tmp_", mamm, "/peak_list_sub.txt"))
...
write.table(peak_list_sub[peak_list_sub$window_id %in% peak_list_sub_filter$window_id,],
            paste0(work_path, "/corr_", mamm, "/peak_list_filter.txt"), row.names=F, col.names=F, sep="\t", quote=F)
...
mamm_list = read.table(paste0(web_path, "/mamm_list.txt"))
...
file_num = length(list.files(
    path = paste0(work_path, "/corr_Mus_musculus/prediction/", mamm),
    pattern = "\\.loc\\.txt\\.gz$"
))
...
for(i in 1:file_num){
    ...
    dat_i = read.table(paste0(work_path, "/corr_Mus_musculus/prediction/", mamm, "/batch_", i,".txt.gz"))
    loc_i = read.table(paste0(work_path, "/corr_Mus_musculus/prediction/", mamm, "/batch_", i,".loc.txt.gz"))
    non_i = read.table(paste0(work_path, "/corr_Mus_musculus/prediction/", mamm, "/batch_", i,".nonzero.txt.gz"))
...
saveRDS(dat, paste0(work_path, "/corr_Mus_musculus/prediction/", mamm, "/dat.rds"))
saveRDS(loc, paste0(work_path, "/corr_Mus_musculus/prediction/", mamm, "/loc.rds"))
...
peak_list = read.table(paste0(work_path, "/tmp_Mus_musculus/peak_list_sub.txt"))
colnames(peak_list) = c("chr", "start", "end")
peak_list$window_id = paste0(peak_list$chr, "_", peak_list$start, "_", peak_list$end)

peak_list_sub = read.table(paste0(work_path, "/corr_Mus_musculus/peak_list_filter.txt"))
...
dat_i = readRDS(paste0(work_path, "/corr_Mus_musculus/prediction/", mamm, "/dat.rds"))
loc_i = readRDS(paste0(work_path, "/corr_Mus_musculus/prediction/", mamm, "/loc.rds"))
...
saveRDS(result, paste0(work_path, "/corr_Mus_musculus/prediction/", mamm, "/result_corr.rds"))
...
for(kk in 1:length(mamm_list)){
    ...
    res_i = readRDS(paste0(work_path, "/corr_Mus_musculus/prediction/", mamm, "/result_corr.rds"))
    x = data.frame(window_ID = names(res_i),
                   corr = as.vector(res_i),
                   species = mamm)
    res = rbind(res, x)
}
saveRDS(res, paste0(work_path, "/corr_Mus_musculus/prediction/result_corr.rds"))
...
### ./corr_mammals/step_3_count_num.sh

x = read.table('candidate_windows_sub_region_num.txt')
sum(x$V1)
...
ggsave("hist_corr_across_mammals_global.pdf", p1 + p2, width = 10, height = 5)
...
for(i in 1:36){
    ...
    tmp = read.table(paste0(work_path, 
                            "/tmp_Mus_musculus/celltype_", i, "_enhancer.bed"))
...
ggsave("hist_mean_corr_across_mammals_split_celltype.pdf", p, width = 15, height = 15)
...
p = ggplot() +
    ...
    theme(axis.text.x = element_text(color="black"), axis.text.y = element_text(color="black")) +
    facet_wrap(~ celltype, nrow = 6, ncol = 6) 
ggsave("hist_median_corr_across_mammals_split_celltype.pdf", p, width = 15, height = 15)


##############################################
### Step-7: subset 100 bp windows, n = 547,317
...
write.table(res_y, paste0(work_path, "/corr_Mus_musculus/100bp_windows_pass_corr_0.6.txt"), row.names=F, sep="\t", quote=F)
...
res_y$median_corr = round(res_y$median_corr, 3)
res_y$mean_corr = round(res_y$mean_corr, 3)
saveRDS(as.data.frame(res_y), paste0(work_path, "/corr_Mus_musculus/window_list.rds"))
saveRDS(as.data.frame(unique(res_y[,colnames(res_y) != 'celltype'])), paste0(work_path, "/corr_Mus_musculus/window_list_uniq.rds"))
