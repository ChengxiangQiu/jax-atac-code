...
###################################################################################
### Step-1: Lifting over those candidate windows to other 240 mammals from Zoonomia
...
Rscript ./help_code/stitchHalFrags_v3.R \
...
mamm_list = []
with open(f"https://shendure-web.gs.washington.edu/content/members/cxqiu/public/backup/jax_atac/download/mamm_list.txt") as file:
    for line in file:
    if line.strip() != mamm:
...
for cnt, mamm_i in enumerate(mamm_list, 1):
    ...
    tmp = set()
    bed_path = f"{work_path}/tmp_{mamm}/liftover/stitchHalFrags_{mamm_i}.bed"
...
output_path = f"{work_path}/tmp_{mamm}/window_list_merged.replicate_count.txt"
...
for(i in 1:36){
    ...
    tmp = read.table(paste0(work_path, 
                            "/tmp_", mamm, "/celltype_", i, "_enhancer.bed"))
    tmp = tmp[,c(1:3)]
...
dat = readRDS(paste0(work_path, "/prediction_", mamm, "/dat.txt.rds"))
peak_list = readRDS(paste0(work_path, "/prediction_", mamm, "/dat_loc.txt.rds"))
...
colnames(peak_list_sub) = c("chr", "start", "end", "window_id")
peak_list_sub_count = read.table(paste0(work_path, "/tmp_", mamm, "/window_list_merged.replicate_count.txt"))
colnames(peak_list_sub_count) = c("window_id", "count")
df = peak_list_sub %>% left_join(peak_list_sub_count, by = "window_id") %>% as.data.frame()
df$count[is.na(df$count)] = 0
...
write.table(dat[keep,], paste0(work_path, "/tmp_", mamm, "/dat_sub.txt"),
            row.names=F, col.names=F, sep="\t", quote=F)
write.table(peak_list[keep, c(1:3)], paste0(work_path, "/tmp_", mamm, "/peak_list_sub.txt"),
            row.names=F, col.names=F, sep="\t", quote=F)
