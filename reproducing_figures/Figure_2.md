...
3D UMAP visualization of the entire dataset (left), epithelial cells (middle), and kidney cells (right).
...
```{code-cell #Fig2a_3D_UMAP_whole_dataset_Left} r
...
```{code-cell #Fig2a_3D_UMAP_whole_dataset_Middle} r
...
```{code-cell #Fig2a_3D_UMAP_whole_dataset_Right} r
...
Heatmap of combined regression coefficients (NNLS; row-scaled) for 13 major scRNA-seq cell clusters (rows) vs. 13 Level-1 scATAC-seq cell lineages.


```{code-cell #Fig2c_NNLS_RNA_ATAC} r
web_path = "https://shendure-web.gs.washington.edu/content/members/cxqiu/public/backup/jax_atac/download/reproducing_figures"
...
Overlap of 36 peak sets from Level-2 scATAC-seq cell classes (rows) vs. 12 peak sets from bulk ATAC-seq of dissected mouse embryonic tissues by ENCODE.


```{code-cell #Fig2d_overlap_ENCODE} r
web_path = "https://shendure-web.gs.washington.edu/content/members/cxqiu/public/backup/jax_atac/download/reproducing_figures"
